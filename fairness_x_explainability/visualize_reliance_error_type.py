...
if __name__ == "__main__":
    ...
    assert args.num_labels == 2, "Only binary classification is supported"
            ...
            for target_class in range(args.num_labels):
                ...
                for group in BIAS_TYPES[args.bias_type]:
                    for i in range(args.num_labels):
                        ...
                        visualization_data[f"class_{i}_group_{group}_labels"] = []
                ...
                for group in BIAS_TYPES[args.bias_type]:
                    ...
                    for prediction, label, counterfactual_predicted_class_confidence_diff, sensitive_reliance in zip(fairness_data[f"{group}_predictions"], fairness_data[f"{group}_labels"], fairness_data[f"{group}_counterfactual_{BIAS_TYPES[args.bias_type][0]}_to_{BIAS_TYPES[args.bias_type][1]}_predicted_class_confidence_diff_list"].values(), sensitive_reliances):
                        ...
                        visualization_data[f"class_{prediction}_group_{group}_labels"].append(label)
                ...
                for RELIANCE_ABS, FAIRNESS_ABS in [(False, False), (True, True)]:
                    ...
                    for group in BIAS_TYPES[args.bias_type]:
                        for i in range(args.num_labels):
                            ...
                            if i != target_class:
                                for label in visualization_data[f"class_{i}_group_{group}_labels"]:
                                    if label==i:
                                        all_groups += [f"{group} True Negative"]
                                    else:
                                        all_groups += [f"{group} False Negative"]
                            else:
                                for label in visualization_data[f"class_{i}_group_{group}_labels"]:
                                    if label==i:
                                        all_groups += [f"{group} True Positive"]
                                    else:
                                        all_groups += [f"{group} False Positive"]
                    ...
                    if not os.path.exists("visualization_reliance_by_error_type"):
                        os.makedirs("visualization_reliance_by_error_type")
                    # save the plot
                    plt.savefig(f"visualization_reliance_by_error_type/{model_type}_{method}_{aggregation}_class_{target_class}_fairness_vs_reliance_fairness_abs_{FAIRNESS_ABS}_reliance_abs_{RELIANCE_ABS}.png")